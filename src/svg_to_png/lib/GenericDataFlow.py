...
class GenericDataFlow(Curve):
    def add_curve(self, d):
        curve = (
            draw.Path(fill="none", stroke="black")
            .M(self.sourceX, self.sourceY)
            .Q(self.controlX, self.controlY, self.targetX, self.targetY)
        )
        d.append(curve)

    def add_additional_arrows(self, d):
        quadratic_interpolator = interpolate_quadratic_bezier(
            [self.sourceX, self.sourceY],
            [self.controlX, self.controlY],
        ...
        quadratic_angle_interpolator = interpolate_quadratic_bezier_angle(
            [self.sourceX, self.sourceY],
            [self.controlX, self.controlY],
            [self.targetX, self.targetY],
        )
        ...
        for i in range(pts):
            ...
            path = (
                draw.Path(
                    fill="black", transform=f"translate({x}, {y}) rotate({angle})"
                )
                .M(12, 0)
                .L(-5, -8)
                .L(0, 0)
    ...
    def add_text(self, d):
        ...
        d.append(rect)
        label = draw.Text(
            self.name, 11, self.handleX - newWidth / 2 + 13, self.handleY + 36
        )
        d.append(label)
    ...
    def add_icon(self, d):
        ...
        image = draw.Image(
            x, y, width, height, data=png_bytes, mime_type="image/PNG", opacity=opacity
        )
        d.append(image)
