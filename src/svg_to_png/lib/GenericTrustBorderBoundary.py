...
class GenericTrustBorderBoundary(Shape):
    ...
    def add_text_old(self, d):
        ...
        for name_part in self.name:
            ...
            ps += p.content
        newWidth, _newHeight = calculate_size(self.name)
        div = draw.Raw(f'<div style="display: table; width: {newWidth}px;">{ps}</div>')
        body = draw.Raw(f"<body>{div.content}</body>")
        foreign_object = draw.ForeignObject(
            body.content,
            x=self.left + self.width - newWidth,
            y=self.top,
            width=self.width,
            height=self.height,
            font_family="Open Sans",
