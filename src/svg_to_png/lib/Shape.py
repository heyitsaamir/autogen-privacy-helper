...
class Shape:
    def __init__(
        self, category, type, name: str, icons: dict, height, width, left, top
    ):
    ...
    def add_text(self, d):
        ...
        # Wrapped text
        lines = wrap_text(
            self.name, self.get_text_line_width(), font_size
        )  # Adjusted max width to fit within the circle
        ...
        for i, line in enumerate(lines):
            text = draw.Text(
                ...
                font_size,
                self.width // 2,
                start_y + i * font_size * 1.2,
                center=self.is_text_centered(),
                valign="middle",
                font_family="Open Sans",
            )
            svg = SVG(
                self.left + self.get_left_indent(),
                self.top,
                self.width - self.get_left_indent(),
                self.height,
            )
    ...
    def add_icon(self, d, x=None, y=None, width=None, height=None):
        ...
        if not icon:
            return
        iconHeight = (
            height if height is not None else max(min(90, self.height / 2.5), 40)
        )
