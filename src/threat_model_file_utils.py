...
def get_threat_model_xml_file(state: AppTurnState):
    if state.temp.input_files and state.temp.input_files[0]:
            if isinstance(state.temp.input_files[0], InputFile):
                if state.temp.input_files[0].content_type == 'application/vnd.microsoft.teams.file.download.info':
    ...
    if state.temp.input_files and state.temp.input_files[0]:
        if isinstance(state.temp.input_files[0], InputFile):
            if state.temp.input_files[0].content_type == 'image/jpeg' or state.temp.input_files[0].content_type == 'image/png':
                return state.temp.input_files[0] # this is not guaranteed to be a threat model image file
    return None