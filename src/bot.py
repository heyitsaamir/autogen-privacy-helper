...
def build_group_chat(context: TurnContext, state: AppTurnState, user_agent: Agent):
    ...
    #     first_attachment = first(context.activity.attachments, lambda attachment: attachment.get('content_type', '') == 'image/*')
    #     if first_attachment is not None:
    #         content_url = first_attachment.content_url
    ...
    # threat_model_validating_agent = AssistantAgent(
    #     name="Threat",
    #     system_message='''You are a helpful threat model validator agent. Your job is to validate the threat model that is sent by the user.''',
...
downloader = TeamsAttachmentDownloader(TeamsAttachmentDownloaderOptions(config.APP_ID, adapter))
