...
class Config:
    ...
    APP_PASSWORD = os.environ.get(
        "BOT_PASSWORD", os.environ.get("SECRET_BOT_PASSWORD", "BAD_PASSWORD")
    )
    ...
    AZURE_LLM_MANAGED_IDENTITY_CLIENT_ID = os.environ.get(
        "AZURE_LLM_MANAGED_IDENTITY_CLIENT_ID"
    )
    ...
    AZURE_MANAGED_IDENTITY_CLIENT_ID = os.environ.get(
    ...
    ENABLE_RUNTIME_LOGGING = (
        os.environ.get("ENABLE_RUNTIME_LOGGING", "false").lower() == "true"
    )
    ...
    def build_llm_config(self):
        if self.OPENAI_KEY:
            ...
            raise ValueError(
                "Neither OPENAI_KEY nor AZURE_OPENAI_KEY nor azure managed identity (AZURE_LLM_MANAGED_IDENTITY_CLIENT_ID, AZURE_LLM_MODEL, AZURE_LLM_BASE_URL) environment variables are set."
            )
        return autogen_llm_config

    def build_cosmos_db_config(self):
        if (
            self.COSMOS_DB_URI is None
            or self.COSMOS_DB_DATABASE_ID is None
            or self.COSMOS_DB_CONTAINER_ID is None
            or (
                self.AZURE_MANAGED_IDENTITY_CLIENT_ID is None
                and self.COSMOS_DB_CONNECTION_STRING is None
            )
        ):
            return None
