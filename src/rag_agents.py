...
class AzureAISearch(VectorDB):
                    ...
                    (
                        {
                            "id": result["id"],
                            "content": " ".join(
                                c.text for c in result["@search.captions"]
                            ),
                            "metadata": None,
                            "embedding": None,
                        },
    ...
    def _search(self, query: str, previous_day_index: int = 0):
        ...
        semantic_search_config = build_config(
            "1-home-index-sc", previous_day_index=previous_day_index
        )
        ...
        search_client = SearchClient(
            search_endpoint, index_name, AzureKeyCredential(api_key)
        )
        try:
            ...
            if previous_day_index <= 2 and isinstance(e, ResourceNotFoundError):
                print(
                    f"Resource not found error. Retrying with previous day. (curr_day - {previous_day_index})"
                )
...
def setup_rag_assistant(llm_config):
    ...
    def message_generator(_recipient, messages, sender, _config):
        ...
        if sender is assistant:
            ...
            return rag_proxy_agent.message_generator(
                rag_proxy_agent, messages, _context
            )
        else:
            return last_msg
    ...
    def trigger(sender):
        return sender not in [
            assistant
        ]  # To prevent the assistant from triggering itself
