...
The data flow is indicated by arrows. Each arrow should have a label inside a green box. The labels should collectively indicate the sequence of the data flow for the entire threat model.
You should ensure that all arrows are sequential and have labels. If any arrow does not have a label, indicate which arrow. If all arrows do not have labels, indicate so in your response.
In your response indicate the sequence of the data flow for the entire threat model (i.e. the data flows from Node 1 to Node 2 to Node 3 etc.) Do not forget to use actual names of nodes rather than Node 1, Node 2 etc.
""")
]
...
class SpecAnswer(BaseModel):
    ...
    steps_to_improve: Annotated[str, "Highly specific steps to improve if any. Use 'None' if no steps to improve"]
...
class EvaluateSpecCapability(AgentCapability):
    ...
    def add_answer(self, answer: SpecAnswer):
...
Answer the questions as clearly and concisely as possible. Always use add_answer to add an answer to a spec question.
    ...
    ThreatModelImageAddToMessageCapability(context, say_when_evaluating=True, state=state, max_width=700).add_to_agent(answerer_agent)
    
    def add_answer(answer: Annotated[SpecAnswer, "The answer to the spec question"]):
        return cap.add_answer(answer)
    ImmediateExecutorCapability().add_to_agent(answerer_agent, add_answer, description="Add an answer to a spec question")
