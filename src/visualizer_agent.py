...
specs = [
    Spec(
        id=1,
        spec="All nodes should be inside a trust boundary",
...
""",
    ...
    Spec(
        id=2,
        spec="The data flow should be easy to understand",
        instructions_to_solve="""
...
Answer the questions as clearly and concisely as possible. Always use add_answer to add an answer to a spec question.
    ...
    ThreatModelImageAddToMessageCapability(
        context, say_when_evaluating=True, state=state, max_width=700
    ...
    def summarize(self, recipient, summary_args):
        ...
        for spec_answer_tuple in spec_answers:
            ...
            spec_answer_improve = (
                f"### Steps to improve\n{
                spec_answer.steps_to_improve}"
                if spec_answer.steps_to_improve != "None"
                else None
            )
